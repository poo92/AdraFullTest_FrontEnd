...
for (var i = 0; i < listing.length; i++) {
  ...
  if (fs.statSync(filename).isFile() && endsWith(filename, '.json') && notImplementedYet.indexOf(path.basename(filename)) === -1) {
          ...
          if (testcase.error !== undefined) {
            ...
            (function(testcase, given) {
              it('should pass test ' + j + " expression: " + testcase.expression, function() {
                assert.deepEqual(search(given, testcase.expression), testcase.result);
              });
            })(testcase, given);
          }
        }
